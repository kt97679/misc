...
start() {
    ...
    printf "%s\n" \
        "#!ipxe" \
	"kernel $vmlinuz ip=dhcp rootfstype=squashfs rd.live.squashimg=$(basename $squashfs) root=live:$squashfs nomodeset console=ttyS0,115200n8 rootsize=10%" \
        "initrd $initrd" \
        "boot" >boot.ipxe
    ...
    $console && cmd_suffix="2>&1|tee qemu.log"
